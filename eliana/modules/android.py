...
def magisk(bot, update):
    ...
    releases = ""
    for type, branch in {"Stable":["master/stable","master"], "Beta":["master/beta","master"], "Canary (debug)":["canary/debug","canary"]}.items():
...
def fw_check(bot, update, args):
    if not len(args) == 2:
        reply = 'Please type your device **model** & **csc**  into it!\nFor example, `/check SM-G975F INS`!'
